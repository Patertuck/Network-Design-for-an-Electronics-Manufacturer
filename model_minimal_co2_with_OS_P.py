...
open_factory = 0
os_list_for_openingcost = [op["LocationID"] for op in OPTIONALSOURCE]
...
for v in decision_vars:
    ...
    if v["part"] == "x":
        ...
        if v["start"] in os_list_for_openingcost:
            open_factory += 1
            os_list_for_openingcost.remove(v["start"])

total_opening_cost = open_factory * OPENINGCOST       

print()
print("------- Costs ----------------")
print()
print(f"The minimal CO2 emission with Optional Sources: {current_optimum}g")        
...
print(f"Open {open_factory} new facilities")
