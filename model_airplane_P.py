...
func1 = sum(list(j["var"] * distance_entry["distance"] * transportcost["air"] for j in decision_vars for distance_entry in distance_data if distance_entry["start"] == j["start"] and distance_entry["end"] == j["dest"]))

func2 = sum(list(j["var"] * sourcingcost[j["start"]] for j in decision_vars if j["part"] == 'z' for distance_entry in distance_data if distance_entry["start"] == j["start"] and distance_entry["end"] == j["dest"]))

func3 = sum(list(j["var"] * handlingcost[j["start"]] for j in decision_vars if j["part"] == 'y' or j["part"] == 'x' for distance_entry in distance_data if distance_entry["start"] == j["start"] and distance_entry["end"] == j["dest"]))
...
for v in decision_vars:
    ...
    transport_cost += v["var"].x * get_distance(distance_data, v["start"], v["dest"]) * transportcost["air"]
...
print()
print(f"The minimal cost to transport with air: {current_optimum}$".format(current_optimum))        
print(f"Cost to compensate CO2 emission: {CO2_emission_cost} $")
print(f"CO2 emission in tonnes: {CO2_emission_cost/CO2PRICE} t")
print(f"Cost to produce and transport the products (without slowness): {sourcing_cost + transport_cost + handling_cost}$")
