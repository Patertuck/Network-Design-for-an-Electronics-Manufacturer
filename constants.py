...
def get_nodes():
    ...
    for i in range(len(nodes_read)):
        ...
        if entry["Role"] == "Retailer":
            ...
            assert False, "In get_nodes, invalid name for Role is found: {}".format(entry["Role"])    
        nodes.append(entry)
...
handlingcost = {
    data["Location"]: data["Value"] for data in data_entries if data["Attribute"] == "Handling cost"
}
print("handlingcost: ", handlingcost)

# linear sourcing cost the each source, keys are locationIDs
sourcingcost = {
    data["Location"]: data["Value"] for data in data_entries if data["Attribute"] == "Sourcing cost"
}

#linear cost to produce one unit at optional source
variablecost = {
    data["Location"]: data["Value"] for data in data_entries if data["Attribute"] == "Variable cost"
}

# binary cost of opening an optional source
openingcost = {
    data["Location"]: data["Value"] for data in data_entries if data["Attribute"] == "OpeningCost"
}

# binary cost of opening an optional source
operationalcost = {
    data["Location"]: data["Value"] for data in data_entries if data["Attribute"] == "OperatingCost"
}


locationCosts = {"z": sourcingcost, "y": handlingcost, "x": handlingcost, "o": variablecost} 
